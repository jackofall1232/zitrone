...
### Decoy traffic (cover traffic) — SHIPPED on Android as of 0.10.0-beta

*(The 2026-07-27 correction that previously stood here — "not built, assume zero cover traffic
protects you today" — is resolved by 0.10.0-beta on Android. The web client's older decoy
generator remains undeployed and statistically distinguishable; nothing below applies to it.)*

**What ships.** On a vault with cover traffic enabled, every real send is paired with a cover
frame of exactly the same length, in unpredictable order, with a randomized per-send delay — so a
network observer cannot tell one real send from a covered pair. The cover envelopes are addressed
to a synthetic relay account provisioned per vault; the synthetic side holds its own relay socket,
acknowledges and burns the cover envelopes it receives, and occasionally answers with a reply-
shaped frame so the exchange is not conspicuously one-directional. **A real message is never
blocked, failed, delayed, reordered, or made less durable to produce cover** — under contention or
failure, cover yields uniformly and silently; the real send always proceeds.
...
**Honest limits, declared rather than implied away:**

- **Dead-air periods are not covered.** Cover is generated by real sends; a client that is idle
  looks idle. This is an accepted, documented limit of 0.10.0 — do not read the feature as
  continuous background cover, because it is not.
- **The plaintext control channel is not covered.** Typing indicators, acks, burns, and receipt
  frames are small plaintext control frames, trivially separable from message traffic by size, and
  this scheme generates no cover for them (spec §2.4). A real conversation also produces inbound
  receipt traffic that a cover exchange does not naturally produce.
- **Relay-visible imperfections exist and are priced in.** A relay that parses cover blobs can find
  tells (declared in full in `docs/design/DECOY_TRAFFIC_0.10.0_SPEC.md` §2.3–2.4: body lengths that
  are not always a padded-block multiple, a synthetic conversation whose counters repeat, a
  `prekey_id` that may name an unpublished id). These are relay-only observations, and the relay is
  conceded in the threat model for stronger reasons than any of them.

### Connection modes

Three user-selectable bundles compose the network layer. **The "Relay hops" column describes
intent, not shipped behavior** — see the correction above. Tor and dead-drop mode are real today.
As of 0.10.0-beta, Android ships send-pairing cover traffic as described in the decoy-traffic
section above; the per-mode "medium/high" decoy levels in this table remain design intent, not a
shipped distinction.
...
## Storage-format stability — read before updating or downgrading
...
*(This wording was reviewed to convergence and ratified by the maintainer 2026-07-27 — spec §4.1
records the full reasoning, including why the answer is disclosure rather than a stability promise:
migrations are not built, and a promise the project has no mechanism to keep would be exactly the
