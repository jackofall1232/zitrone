...
class WsSyntheticSocketTest {
    ...
    /**
     * No socket is opened: [WsSyntheticSocket] installs its listener in its own initialiser, so the
     * routing table is reachable through `socket.listener` without a relay. The class takes no
     * [WsClient] — it builds one — which is what makes "the real socket cannot be injected here" a
     * fact about the type rather than something a test has to assert.
     */
    private fun socket(onRateLimited: () -> Unit = {}) = WsSyntheticSocket(
    ...
    fun `a delivery is forwarded to the session's callback`() {
        ...
        socket.listener.onMessageDeliver(envelope())
