...
class VaultCapacityFixture(ops: VaultSodiumOps) {
    ...
    fun stateFilledToCap(): VaultState {
        ...
        repeat(8) {
            ...
            if (slack in 0..MAX_SLACK_BYTES) return stateWithBlob(size)
            // Incompressible content, so encoded size tracks blob size almost exactly; aim just
            // under the cap and converge.
            size += slack - MAX_SLACK_BYTES / 2
            check(size in 1 until VaultStateCodec.MAX_PAYLOAD_CONTENT_BYTES) { "fill diverged: $size" }
        }
        throw AssertionError("could not converge on a state at the capacity boundary")
    }
