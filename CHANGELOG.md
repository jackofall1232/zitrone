...
paired with a cover frame of exactly the same length, in unpredictable order, with a randomized
per-send delay — a passive network observer (ISP, hostile Wi-Fi, hostile Tor exit) sees frame
sizes and timings that no longer distinguish a real send from a covered pair. The cover envelopes
go to a synthetic relay account provisioned per vault; the synthetic side holds its own socket,
acknowledges and burns what it receives, and occasionally answers with a reply-shaped frame so the
exchange is not conspicuously one-directional. Built across four independently reviewed units
(provisioning, envelope construction, send pairing, synthetic receive) totalling over twenty
adversarial review rounds; the absolute rule throughout — **a real message is never blocked,
failed, delayed, reordered, or made less durable to produce cover** — is enforced in code and
pinned by tests, and under contention or failure, cover yields uniformly and silently.

**Honest limits, stated up front** (full detail: `docs/SECURITY_MODEL.md` and
`docs/design/DECOY_TRAFFIC_0.10.0_SPEC.md` §2.3–2.4):

- **Dead-air periods are not covered.** Cover is generated by real sends; an idle client looks
  idle. This is an accepted, documented limit — 0.10.0 is not continuous background cover.
- **The plaintext control channel (typing, acks, burns, receipts) is not covered.**
- **The relay is conceded**, as it always has been: it sees cleartext sender/recipient ids on
  every envelope, and cover traffic does not and cannot change that. Relay-visible imperfections
  in the cover blobs themselves are declared in the spec rather than papered over.

**⚠️ Storage-format disclosure — read before updating:**
...
**Maturity, honestly:** the `-beta` in the version string is a label kept for continuity; by the
project's own assessment these are **pre-beta (alpha) builds**. The flip to a true beta is planned
after the 0.11.0 polish round.
